...
module Actors = 
    ...
    type TypedActorRef<'Message>(underlyingRef : IActorRef) = 
        ...
        interface ActorRef<'Message> with
            
            /// <summary>
            /// Changes the type of handled messages, returning new typed ActorRef.
            /// </summary>
            member __.Switch<'T>() = TypedActorRef<'T>(underlyingRef) :> ActorRef<'T>
    ...
    /// </summary>
    let inline typed (actorRef : IActorRef) : ActorRef<'Message> = 
        (TypedActorRef<'Message> actorRef) :> ActorRef<'Message>
    ...
    type TypedActorSelection<'Message>(selection : ActorSelection) = 
        ...
        /// </summary>
        member __.Anchor with get (): ActorRef<'Message> = typed selection.Anchor
        ...
        /// </summary>
        member __.Elements with get () = selection.Elements

        /// <summary>
        /// Sets collection of elements, actor selection path is build from.
        /// </summary>
        member __.Elements with set (e) = selection.Elements <- e

        /// <summary>
        /// Tries to resolve an actor reference from current actor selection.
        /// </summary>
        member __.ResolveOne (timeout: TimeSpan): Async<ActorRef<'Message>> = 
    ...
    /// </summary>
    let inline (<!) (actorRef : #CanTell<'Message>) (msg : 'Message) : unit = 
    ...
    /// </summary>
    let inline (<?) (tell : #CanTell<'Message>) (msg : 'Message) : Async<'Response> = tell.Ask<'Response>(msg, None)
    
    /// Pipes an output of asynchronous expression directly to the recipients mailbox.
    let pipeTo (computation : Async<'Message>) (recipient : CanTell<'Message>) (sender : IActorRef) : unit = 
    ...
    let inline (|!>) (computation : Async<'Message>) (recipient : CanTell<'Message>) = 
        pipeTo computation recipient ActorRefs.NoSender
    
    /// Pipe operator which sends an output of asynchronous expression directly to the recipients mailbox
    let inline (<!|) (recipient : CanTell<'Message>) (computation : Async<'Message>) = 
        pipeTo computation recipient ActorRefs.NoSender
    ...
    type Actor<'Message> = 
        ...
        /// </summary>
        abstract Self : ActorRef<'Message>
        ...
        /// </summary>
        abstract Sender<'Response> : unit -> ActorRef<'Response>
    ...
    type FunActor<'Message, 'Returned>(actor : Actor<'Message> -> Cont<'Message, 'Returned>) as this = 
        ...
        let mutable state = 
            ...
            actor { new Actor<'Message> with
                        ...
                        member __.Sender<'Response>() = typed (this.Sender()) :> ActorRef<'Response>
