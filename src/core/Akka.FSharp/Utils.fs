...
module EventStreaming = 
    ...
    /// </summary>
    let subscribe (ref : ActorRef<'Message>) (eventStream : Akka.Event.EventStream) : bool = 
        eventStream.Subscribe(ref, typeof<'Message>)
    
    /// <summary>
    /// Unubscribes an actor reference from target channel of the provided event stream.
    /// </summary>
    let unsubscribe (ref : ActorRef<'Message>) (eventStream : Akka.Event.EventStream) : bool = 
