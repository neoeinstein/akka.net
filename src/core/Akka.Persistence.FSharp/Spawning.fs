...
module Spawn =
    ...
    let spawnPersist actorFactory options (name : PersistenceId) (state : 'State)
        (aggregate : Eventsourced<'Command, 'Event, 'State> -> Aggregate<'Command, 'Event, 'State>) : ActorRef<'Command> =
    ...
    /// <param name="options">Additional spawning options.</param>
    let spawnView actorFactory options (viewName : PersistenceId) (name : PersistenceId)
        (f : View<'Message, 'State> -> Cont<'Message, 'Returned>) : ActorRef<'Command> =
        (fun () -> new FunPersistentView<'Event, 'State>(f, name, viewName))
        |> Linq.PersistentExpression.ToExpression
        |> createFromExpression actorFactory options name
    ...
    /// <param name="options">Additional spawning options.</param>
    let spawnDeliverer actorFactory options (name : PersistenceId) (state : 'State)
        (aggregate : Delivery<'Command, 'Event, 'State> -> DeliveryAggregate<'Command, 'Event, 'State>) : ActorRef<'Command> =
